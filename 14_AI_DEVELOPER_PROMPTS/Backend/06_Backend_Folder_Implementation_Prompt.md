...
- Start from `00_START_HERE/Current_Source_Of_Truth.md`; follow and actually
  ...
  path alone is not a read. Search current backend structure and complete the
  Reuse/Extend/New assessment before changing folders.
...
```text
You are a senior .NET backend architect working on OneVerz POS MVP.
