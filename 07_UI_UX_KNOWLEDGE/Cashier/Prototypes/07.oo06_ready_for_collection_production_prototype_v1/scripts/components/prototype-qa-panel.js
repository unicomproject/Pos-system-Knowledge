  ...
  render(uiState, permissionService){
    ...
    return `
      ...
      <aside class="qa-panel" id="qaPanel">
        ...
        <div class="qa-row">
          <label for="qaNotifyState">Notify result</label>
          <select id="qaNotifyState">
            ${["Idle","Success","Error"].map(v=>`<option ${uiState.notifyState===v?"selected":""}>${v}</option>`).join("")}
          </select>
        </div>
        ...
        <div class="qa-note">
          Prototype-only. Used to validate states, permission/entitlement coverage, responsive behavior and long-content handling.
        </div>
      </aside>
    `;
  }
