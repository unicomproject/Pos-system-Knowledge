OV.AppShell = {
  ...
  mobileContext(){
    return `
      <div class="mobile-context-strip" aria-label="POS context">
  ...
  bottomNav(){
    return `
      <nav class="bottom-nav" aria-label="Primary navigation">
        ${this.nav("home","Home")}
        ${this.nav("cart","New Sale")}
        ${this.nav("orders","Orders",true)}
        ${this.nav("customer","Customers")}
        ${this.nav("settings","Settings")}
      </nav>
    `;
  },

  nav(icon,label,active=false){
    return `<button class="nav-item ${active?"active":""}">${OV.Icon(icon)}<span>${label}</span></button>`;
  }
};
