...
<!-- status: PROTOTYPE APPROVED / UI-UX CONTRACT LOCKED v1.0 -->
...
This document is the locked visual/UX master specification for the current 29-screen Inventory implementation. It does not start Flutter/.NET work.
...
## HTML/CSS source is not a production constraint
...
Production Flutter may use appropriate reusable widgets and architecture.

The final rendered result must conform to the locked UI/UX contract.
...
Production Tenant Admin shell: black approved Tenant Admin shell (`TenantAdminSharedShell`).

Inventory navigation: Inventory is a top-level Tenant Admin navigation item.

Prototype: Inventory workspace/content area remains the visual contract.

The prototype's obsolete/outdated outer chrome (Settings-nested white sidebar, POS till session chrome, POS bottom nav) must NOT override the canonical Tenant Admin production shell.


## Shared Visible Chrome (all 29 screens)

Documented from the HTML prototypes. **NOT LOCKED** as production chrome (see Production shell rule).
...
## Implementation audit gaps (closed — contract locked)

**Closed in the implementation audit (2026-08-15).** Canonical closure: [[Tenant_Admin_Inventory_Implementation_Audit]]. Lock: [[Tenant_Admin_Inventory_Lock_Manifest]].

Do not re-open these as undecided. UI/UX Contract is **LOCKED**. Historical gap text below is retained for traceability.
...
```text
IMPLEMENT LOCKED 29-SCREEN INVENTORY FLUTTER UI
```

Do not start Flutter/.NET automatically from this document. Frontend implementation is READY TO START against this lock.
