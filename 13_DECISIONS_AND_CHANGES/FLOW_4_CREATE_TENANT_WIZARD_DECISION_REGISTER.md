...
| F4-D28 | Malware-scan availability | Submission may record an unavailable scanner outcome for operational recovery, but approval is fail-closed: only evidence whose current scan result is `CLEAN` can authorize payment. A positive scan is rejected before private storage. |
| F4-D29 | Activation trust chain | Legacy invoice/payment `MarkPaid` is not sufficient activation authority. Activation locks and validates the exact tenant, subscription, invoice, approved payment, expected amount and currency chain, then queues at most one invitation request. |
...
| F4-C17 Payment delivery | Earlier Flow 4/email docs required a provider payment link and the audited worker reported `payment_provider_not_configured` vs approved manual collection | Canonical Flow 4/API/data/test docs; onboarding worker; payment link/transaction entities | Current release could not complete paid onboarding and could be misrepresented as needing a gateway | Manual instructions + secure status/evidence/review; `checkoutUrl` null; future adapter contract retained | Implemented by `db9d579`; manual payment architecture/API/data/test evidence updated | Resolved |
