...
| `requested_collection_at` | timestamptz |  | NULL | Requested click-and-collect pickup window start in UTC. |
| `requested_collection_end_at` | timestamptz |  | NULL | Requested click-and-collect pickup window end in UTC. |
| `collection_timezone_snapshot` | varchar(80) |  | NULL | Outlet timezone used when the requested collection window was selected. |
...
For storefront click-and-collect, confirmation snapshots the requested collection window from `checkout_sessions` into `sales_orders`. This is a customer preference window generated from outlet configuration; it is not a pickup-slot capacity booking in the current release.
