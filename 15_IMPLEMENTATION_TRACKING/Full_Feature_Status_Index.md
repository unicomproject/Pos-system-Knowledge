...
| Full Stack | CatalogProduct | External Product Lookup + Category/Brand Mapping + Quick Add Brand | Completed | 2026-09-23 | Current working tree | OpenFoodFacts (default enabled) + UPCitemdb (controlled fallback, prod disabled/dev enabled) provider-priority lookup with provider-scoped cache; External Category Mapping and External Brand Mapping (normalized text key, ADR 011) with transactional Product-create persistence; Flutter Quick Add Brand. Full Product Setup E2E validated live against real Postgres + live OpenFoodFacts. [[Backend/CatalogProduct/External_Product_Enrichment_Implementation_Status]] |
