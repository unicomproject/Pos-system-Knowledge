...
| Backend | CatalogProduct | Tenant Admin Product Setup — scanner-first B1–B12 + CURRENT Steps 2–7 | Backend Complete for Steps 2–4/6 + VARIANT Step 5; BUNDLE Step 5 PARTIAL / Flutter journey PARTIAL (provider blocked) | 2026-09-13 | [[99_AUDITS/PRODUCT_SETUP_SUPPLIED_USER_JOURNEY_GAP_MATRIX_2026-09-13]] | User-supplied journey Flutter Step 1 S1-A…R3 implemented; Create Duplicate wired; **REAL EXTERNAL FOUND DATA BLOCKED** (zero providers). BUNDLE graph OUT OF SCOPE. |
| Cross-platform | CatalogProduct | Tenant Admin Add Product CURRENT Step 4 — Units & Pack Conversion (LEGACY: FORMER Step 3 of 7-step; TARGET 6-step: absorbed into Step 3 Product Type & Configuration) | Backend IMPLEMENTED / Flutter pending | 2026-09-13 | [[04_MODULE_KNOWLEDGE/10_Product_Core/Tenant_Admin_Product_Units_Pack_Conversion_Specification]] | Backend draft Units path + validators + GET hydrate proven. VARIANT+TrackInventory ON routes to Step 4 (`ProductWizardNextStageResolver`). BUNDLE NOT_APPLICABLE (advance+read+write reject). Flutter UI pending. |
| Cross-platform | CatalogProduct | Tenant Admin Add Product Step 5 — Variant Configuration (LEGACY: FORMER Step 4 of 7-step; TARGET 6-step: Step 3 Product Type & Configuration owns Variant Matrix) | Backend IMPLEMENTED (VARIANT) / BUNDLE PARTIAL / Flutter pending | 2026-09-13 | [[04_MODULE_KNOWLEDGE/12_Product_Option_Variant_Configuration/Tenant_Admin_Product_Variant_Configuration_Specification]] | VARIANT Cartesian/count/hash/tombstones/`ProjectVariantConfigurationAsync` + B10 identifiers IMPLEMENTED. BUNDLE component graph draft persist still PARTIAL. Flutter Step 5 UI pending. |
