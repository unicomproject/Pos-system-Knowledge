...
| F4-GAP-004 | F4-REQ-056,063 | LIVE_SERVICE_GAP — PARTIAL ENV PROGRESS (Chunk 5E) | ACS User Secrets + sender remain PASS. Chunk 5E provisioned isolated Postgres (`oneverz-flow4-live-email-pg` / `oneverz_flow4_e2e_liveemail`) with EF migrations and unexpired `flow4_test_control.environment_marker`, then removed the container after evidence. Controlled mailbox, recipient allow-list and approved HTTPS payment/setup base URLs remain unavailable. No live ACS send, inbox receipt or Playwright 21-pass (Chunk 5E). Earlier Chunk 5–5D `BLOCKED_EXTERNAL` evidence is retained. | Email, outbox, operations | P0 | Provider acceptance + mailbox still unproven | Supply controlled mailbox + allow-list + approved HTTPS bases; re-provision isolated DB/marker for the live session; then capture ACS operation IDs, inbox evidence and Playwright 21 with zero skips. See [[FLOW_4_LIVE_ACS_MAILBOX_AND_PLAYWRIGHT_COMPLETION_EVIDENCE_2026-08-05]] |
