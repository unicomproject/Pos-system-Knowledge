...
| F4-GAP-004 | F4-REQ-056,063 | LIVE_SERVICE_GAP — PARTIAL CREDENTIAL PROGRESS (Chunk 5D) | ACS connection string + sender are present in approved `.NET User Secrets`; ACS endpoint host DNS/TCP 443 passes; `AllowAdminSecureLinkFallback=false`. Controlled mailbox, recipient allow-list, approved HTTPS payment/setup base URLs and isolated Flow 4 live DB remain unavailable. No live ACS send or inbox receipt was executed (Chunk 5D). Earlier Chunk 5/5B/5C `BLOCKED_EXTERNAL` evidence is retained. | Email, outbox, operations | P0 | Provider acceptance + mailbox still unproven | Supply controlled mailbox + allow-list + HTTPS bases + isolated Test/E2E DB; then capture ACS operation IDs, inbox evidence and Playwright 21 with zero skips. See [[FLOW_4_LIVE_ACS_CREDENTIALED_EXTERNAL_RERUN_EVIDENCE_2026-08-05]] |
