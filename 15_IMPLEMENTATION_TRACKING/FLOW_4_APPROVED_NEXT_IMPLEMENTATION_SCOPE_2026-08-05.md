...
| 5 | F4-GAP-004 | F4-REQ-044,051–056,063 | Live ACS payment and invitation validation — **PARTIAL (Chunk 5E)** | Deployment environment; Backend/Angular QA only as needed | ACS secrets + isolated DB/marker provisioned 2026-08-05; still missing controlled mailbox, allow-list, approved HTTPS bases | Payment-required/outcome/activation invite, failure/retry/resend, provider accepted ID, recipient route, redacted logs; delivery reported separately | Sanitized provider-operation evidence and staging mailbox/browser artifacts; zero secrets. See [[FLOW_4_LIVE_ACS_MAILBOX_AND_PLAYWRIGHT_COMPLETION_EVIDENCE_2026-08-05]] (`CONDITIONAL_GO_TO_CHUNK_6`) |
