...
## Tenant Role Setup Runtime Correction - 2026-08-21

The Tenant Admin role aggregate implements setup options, permission catalog,
permission and assignment replacement, and atomic `PUT roles/{roleId}/setup`.
The setup-options contract exposes only `TENANT_ADMIN` and `CASHIER`.

For any editable role, active persisted permissions must be represented as
assignable items or explicit locked-and-preserved items. If that safe
representation is not possible, final save must reject without changing the
active permission set.

Status: `BLOCKED`. Runtime validation found 44 active Cashier grants without
matching catalog entries. Catalog/seed/entitlement/delegation reconciliation is
required before enabling production role editing.
