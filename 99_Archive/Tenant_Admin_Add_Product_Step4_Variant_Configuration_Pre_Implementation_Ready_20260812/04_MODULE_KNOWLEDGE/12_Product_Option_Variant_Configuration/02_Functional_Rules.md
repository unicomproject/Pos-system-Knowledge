...
- **Add Product Step 4 Variant Configuration**:
  ...
  - Delete Variant Action: Archives combination tombstone (`status = 'ARCHIVED'`). Operational variants with history cannot be deleted.
...
| Tenant Admin | May manage tenant-owned configuration only when entitlement (`product_catalog`) and permissions (`catalog.products.create` / `catalog.products.update` + `catalog.variants.manage`) pass |
...
- Recompute Cartesian variant matrix server-side upon Save Draft / Save & Continue.
