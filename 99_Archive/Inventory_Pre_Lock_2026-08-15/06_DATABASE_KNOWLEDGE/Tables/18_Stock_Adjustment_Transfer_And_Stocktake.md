...
- The uploaded ERD image shows `stock_adjustment_lines.stock_adjustment_id` and `stocktake_sessions.generated_stock_adjustment_id` as FKs, but the `stock_adjustments` entity box is not visible in this image.
- **2026-08-15 Inventory audit:** header attributes are now specified in `Inventory_29_Screen_Schema_Mapping.md` (`stock_adjustments`). Do not treat the missing ERD box as an open blocker. No migration in the audit phase.
