...
- **Save & Continue**: Filled primary button (`Icons.arrow_forward`). Validates Step 2 tracking matrix, persists atomically, and advances to Step 3 (or Step 4/5 based on backend target step).

---

## 4.1 Step 3 Form Layout — Units & Pack Conversion Setup

- **Page Title**: Units & Pack Conversion
- **Page Subtitle**: Define how this product is purchased, sold, and counted in inventory.

### Unit Model Selection (Radio / Segmented Toggle)
- Options:
  1. **Single Unit Only** (`SINGLE_UNIT`): "Item is purchased, sold, and counted in one single unit."
  2. **Multiple Units & Pack Conversion** (`MULTIPLE_UNITS`): "Item has a multi-tier package structure (e.g. Piece, Pack, Carton)."

### Single Unit Only Mode Controls:
- **Product Unit \***: Dropdown Select (bound to UOM master options: Piece, Each, Kilogram, etc.).
- **Allow Decimal Quantity**: Switch toggle with helper text ("Allow fractional quantities like 1.5 kg").
- **Info Guidance Card**: Light gray card (#F9FAFB) displaying: *"This product will be purchased, sold, and counted in Piece. No pack conversion is applied."*

### Multiple Units & Pack Conversion Mode Controls:
- **Grid Layout**: 2-Column responsive form:
  - **Base Unit \***: Dropdown Select. Smallest stock ledger unit (e.g. Piece).
  - **Selling Unit \***: Dropdown Select. Default POS/Online sale unit (e.g. Piece).
  - **Purchase Unit \***: Dropdown Select. Default receiving unit (e.g. Pack). Must differ from Base Unit.
  - **Items per Purchase Unit \***: Numeric input field (> 0). (e.g. 6).
  - **Outer Pack Unit (Optional)**: Dropdown Select. Bulk package level (e.g. Carton).
  - **Purchase Units per Outer Pack**: Numeric input field (> 0, required if Outer Pack selected). (e.g. 12).
  - **Allow Decimal Quantity**: Switch toggle.

### Conversion Summary Card (Dynamic Live Card):
Container background `#EFF6FF` (light blue tint), border `1px solid #BFDBFE`, padding `16px`, border-radius `8px`.
- Renders live conversion math string:
  - `1 Pack = 6 Pieces`
  - `1 Carton = 12 Packs`
  - `1 Carton = 72 Pieces`
  - *Inventory stock ledger is tracked in Pieces.*

### Units & Pack Conversion Table:
- Data table rendering configured unit hierarchy:
  - Columns: `Unit Level`, `Unit Name`, `Conversion to Base Unit`, `Barcode`, `Status`.
  - Barcode column renders placeholder badge: `Assigned in Step 5`.

### Footer Actions on Step 3
- **Back**: Primary outline button (`Icons.arrow_back`). Navigates to Step 2.
- **Save Draft**: Outlined primary button (`Icons.save_outlined`). Persists Step 3 without advancing.
- **Skip**: **DISABLED / HIDDEN** when Track Inventory = ON. (Step 3 is mandatory for inventoried items).
- **Save & Continue**: Filled primary button (`Icons.arrow_forward`). Validates Step 3, persists atomically, and navigates to target step returned by backend (`Step 5` for Simple, `Step 4` for Variant/Bundle).
