...
## Tenant Admin User Creation 5-Step Mapping — 2026-08-25
...
The create request accepts one role, outlet IDs, optional additive direct permission IDs, profile media asset ID, invite intent, and `INACTIVE`/`INVITED` status. It does not accept user-specific till IDs, default till, save-draft state, access start date, 2FA selection, or plaintext temporary password. `Idempotency-Key` is required. See [[../03_USER_JOURNEYS/Tenant_Admin/07_User_Management_Add_New_User_Flow]].
