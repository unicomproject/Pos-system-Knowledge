...
| **BUNDLE $\rightarrow$ SIMPLE** | Destroys Kit Component mappings | Invalidates Step 4 Kit Assembly. Clears `bundle_components` records. Resets Step 4 to N/A. | **YES** ("Changing to Simple Product will remove all bundle component selections. Proceed?") |
...
| **BUNDLE $\rightarrow$ VARIANT** | Destroys Components, requires Variant Matrix | Clears `bundle_components` mappings. Switches Step 4 to Variant Matrix mode. | **YES** ("Changing from Bundle to Variant will remove all bundle components. Proceed?") |
