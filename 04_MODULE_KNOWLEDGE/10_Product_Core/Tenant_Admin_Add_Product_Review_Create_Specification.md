...
> **Canonical wizard (LOCKED):** 1 Scan Barcode → 2 Basic Details → 3 Product Type & Tracking → 4 Unit & Pack → 5 Product Configuration (incl. identifiers) → 6 Pricing & Tax → 7 Review & Create.  
...
## 1. Overview
This document serves as the canonical source of truth for the Tenant Admin Add Product **Step 7: Review & Create**. It replaces all older 8-step wizard references and "Channel Visibility" standalone step concepts. The wizard is strictly 7 steps (scanner-first).
...
- **Actual Persisted Values Only:** Only fields that have been saved in Steps 1–6 are displayed. Skipped steps or not-applicable structures do not display fake data. Step 1 Scan may show acquisition/bootstrap context when a draft scan context exists.
...
- Edit returns to Step 1 Scan Barcode
...
Initial Tracking Details are **not** Basic Details fields. Show remaining applicable
identity values with Product Type & Tracking (section C), after Step 3
reconciliation.

### C. Product Type & Tracking
...
- **FR-RC-014b:** **Add Another Product** starts a fresh Add Product wizard at Step 1 Scan Barcode.
...
- **BR-RC-014:** 7-step workflow is authoritative (scanner-first).
...
## 9. API Contract
- **Review Snapshot API:** `GET /api/v1/tenant-admin/products/{productId}/setup` (Returns `ProductSetupWizardDto` with validation checklist; hydrate scan context when present)
...
| Track Inventory | 3 | TrackInventory | SaveProductDraftRequest | ProductInventorySetting | product_inventory_settings.is_stock_tracked | TrackInventory | Stock Visibility |
...
| Unit Fields | 4 | UnitModel etc. | SaveProductDraftRequest | ProductUnitSetting | product_unit_settings.* | BaseUnitName etc. | UOM Display |
| Variant/Bundle Info | 5 | VariantConfiguration | SaveProductDraftRequest | ProductVariant | product_variants.* | VariantConfiguration | Variant Count |
| SKU | 5 | IdentifierConfiguration | SaveProductDraftRequest | ProductVariant | product_variants.sku | Sku | SKU Search |
| Barcode | 5 | IdentifierConfiguration | SaveProductDraftRequest | ProductBarcode | product_barcodes.barcode | Barcode | Barcode Scan |
...
| Initial Batch Number | 3 | InitialBatchNumber | SaveProductDraftRequest | Draft tracking | EXISTING product_setup_initial_tracking.initial_batch_number | InitialBatchNumber | None (identity, not list qty) |
| Initial Expiry Date | 3 | InitialExpiryDate | SaveProductDraftRequest | Draft tracking | EXISTING product_setup_initial_tracking.initial_expiry_date | InitialExpiryDate | None |
| Initial Serial Number | 3 | InitialSerialNumber | SaveProductDraftRequest | Draft tracking | EXISTING product_setup_initial_tracking.initial_serial_number | InitialSerialNumber | None |
